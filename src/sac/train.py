...
if __name__ == "__main__":
    ...
    model = GNNMASAC(env.observation_space, 64, 1, 1e-3, mem_capacity=1000)
    ...
    for episode in range(max_episodes):
        ...
        while not done:
            ...
            model.update(batch_size, tot_step)
            step += 1
            tot_step += 1
            print(f"Episode {episode}, Step {step}, Reward {rewards}, Done {done}, Info {info}")
            if tot_step % save_interval == 1:
                model.save(tot_step)
                vec_env.save(f"{model.path_prefix}vec_env_{tot_step}.pkl")
    model.save("final")
    vec_env.save(f"{model.path_prefix}vec_env_final.pkl")
