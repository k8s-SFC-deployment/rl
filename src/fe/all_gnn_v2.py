...
class ALLGNNFE(BaseFeaturesExtractor):
    def __init__(self,  observation_space: spaces.Dict, hidden_dim: int = 16, output_dim: int = 32):
        ...
        self.conv1 = HeteroConv({
            ("node", "communicate", "node"): GATConv(self.in_channels_dict["node"], hidden_dim, edge_dim=self.edge_dim_dict[("node", "communicate", "node")], add_self_loops=False),
            ("pod", "is_in", "node"): GATConv(self.in_channels_dict["node"], hidden_dim, add_self_loops=False),
            ("node", "have", "pod"): GATConv(self.in_channels_dict["pod"], hidden_dim, add_self_loops=False),
            ("pod", "belong_to", "service"): GATConv(self.in_channels_dict["pod"], hidden_dim, add_self_loops=False),
            ("service", "include", "pod"): GATConv(self.in_channels_dict["pod"], hidden_dim, add_self_loops=False),
            ("service", "communicate", "service"): GATConv(self.in_channels_dict["service"], hidden_dim, edge_dim=self.edge_dim_dict[("service", "communicate", "service")], add_self_loops=False),
        }, aggr="sum")

        # hidden_channel -> output_dim
        self.conv2 = HeteroConv({
            ("node", "communicate", "node"): GATConv(hidden_dim, output_dim, edge_dim=self.edge_dim_dict[("node", "communicate", "node")], add_self_loops=False),
            ("pod", "is_in", "node"): GATConv(hidden_dim, output_dim, add_self_loops=False),
            ("node", "have", "pod"): GATConv(hidden_dim, output_dim, add_self_loops=False),
            ("pod", "belong_to", "service"): GATConv(hidden_dim, output_dim, add_self_loops=False),
            ("service", "include", "pod"): GATConv(hidden_dim, output_dim, add_self_loops=False),
            ("service", "communicate", "service"): GATConv(hidden_dim, output_dim, edge_dim=self.edge_dim_dict[("service", "communicate", "service")], add_self_loops=False),
        }, aggr="sum")
