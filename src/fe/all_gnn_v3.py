...
class ALLGNNFE(BaseFeaturesExtractor):
    def __init__(self,  observation_space: spaces.Dict, hidden_dim: int = 16, output_dim: int = 32):
        ...
        self.conv1 = HeteroConv({
            ("node", "communicate", "node"): GATv2Conv(self.in_channels_dict["node"], hidden_dim, edge_dim=self.edge_dim_dict[("node", "communicate", "node")], add_self_loops=False),
            ...
            ("service", "communicate", "service"): GATv2Conv(self.in_channels_dict["service"], hidden_dim, edge_dim=self.edge_dim_dict[("service", "communicate", "service")], add_self_loops=False),
        }, aggr="sum")

        # hidden_channel -> output_dim
        self.conv2 = HeteroConv({
            ("node", "communicate", "node"): GATv2Conv(hidden_dim, output_dim, edge_dim=self.edge_dim_dict[("node", "communicate", "node")], add_self_loops=False),
            ...
            ("service", "communicate", "service"): GATv2Conv(hidden_dim, output_dim, edge_dim=self.edge_dim_dict[("service", "communicate", "service")], add_self_loops=False),
        }, aggr="sum")
